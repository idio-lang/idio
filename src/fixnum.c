...
static void idio_fixnum_conversion_error (char const *msg, IDIO num, IDIO c_location)
{
    ...
    idio_error_raise_cont (idio_condition_rt_fixnum_conversion_error_type,
...
static void idio_fixnum_number_error (char const *msg, IDIO num, IDIO c_location)
{
    ...
    idio_error_raise_cont (idio_condition_rt_fixnum_error_type,
			   IDIO_LIST4 (idio_get_output_string (msh),
				       idio_get_output_string (lsh),
				       idio_get_output_string (dsh),
				       num));
...
IDIO_DEFINE_FIXNUM_CMP_PRIMITIVE_(ne, !=)
...
IDIO_DEFINE_ARITHMETIC_CMP_PRIMITIVE1V ("ne", ne)
...
IDIO_DEFINE_ARITHMETIC_BINARY_CMP_PRIMITIVE ("binary-ne", binary_ne, ne)
...
{
    idio_module_set_symbol_value (IDIO_SYMBOLS_C_INTERN ("FIXNUM-MAX"), idio_fixnum (IDIO_FIXNUM_MAX), idio_Idio_module);
    idio_module_set_symbol_value (IDIO_SYMBOLS_C_INTERN ("FIXNUM-MIN"), idio_fixnum (IDIO_FIXNUM_MIN), idio_Idio_module);
