...
void idio_handle_read_error (IDIO h, IDIO c_location)
{
    ...
    idio_error_raise_cont (idio_condition_io_read_error_type,
...
void idio_handle_write_error (IDIO h, IDIO c_location)
{
    ...
    idio_error_raise_cont (idio_condition_io_write_error_type,
			   IDIO_LIST4 (idio_get_output_string (msh),
				       idio_get_output_string (lsh),
				       idio_get_output_string (dsh),
				       IDIO_HANDLE_PATHNAME (h)));
...
void idio_handle_closed_error (IDIO h, IDIO c_location)
{
    ...
    idio_error_raise_cont (idio_condition_io_closed_error_type,
			   IDIO_LIST4 (idio_get_output_string (msh),
				       idio_get_output_string (lsh),
				       idio_get_output_string (dsh),
				       IDIO_HANDLE_PATHNAME (h)));
...
IDIO idio_load_handle (IDIO h, IDIO (*reader) (IDIO h), IDIO (*evaluator) (IDIO e, IDIO eenv), IDIO eenv, int preserve)
{
	...
	if (handle_interactive) {
	     ...
	     */
	    idio_vm_invoke_C (idio_module_symbol_value_thread (thr,
							       IDIO_SYMBOLS_C_INTERN ("do-job-notification"),
