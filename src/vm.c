...
static void idio_vm_invoke (IDIO thr, IDIO func, int tailp)
{
    ...
    case IDIO_TYPE_PRIMITIVE:
	{
	    ...
	    if (idio_S_nil != last) {
		idio_error_C ("primitive: varargs?", last, IDIO_C_LOCATION ("idio_vm_invoke"));
    ...
    case IDIO_TYPE_CONTINUATION:
	{
	    ...
	    if (idio_S_nil != last) {
		idio_error_C ("continuation: varargs?", last, IDIO_C_LOCATION ("idio_vm_invoke"));
...
static idio_ai_t idio_vm_get_or_create_vvi (idio_ai_t mci)
{
	 ...
	 * gvi of 0 so that IDIO_A[_CHECKED]_GLOBAL_FUNCTION_REF (and
	 * IDIO_A[_CHECKED]_GLOBAL_REF) can return the symbols for
...
int idio_vm_run1 (IDIO thr)
{
    ...
    case IDIO_A_GLOBAL_REF:
	{
	    ...
	    IDIO_VM_RUN_DIS ("GLOBAL-REF %" PRId64 " %s", mci, IDIO_SYMBOL_S (sym));
		...
		if (idio_S_undef == val) {
		    ...
		    idio_error_printf (IDIO_C_LOCATION ("idio_vm_run1/GLOBAL-REF"), "unspecified toplevel: %" PRId64 "", mci);
		...
		if (idio_S_unspec == sk_ce) {
		    ...
		    if (0 == sk_gvi) {
			idio_error_runtime_unbound (fmci, fgci, sym, IDIO_C_LOCATION ("idio_vm_run1/GLOBAL-REF"));
	...
	break;
    case IDIO_A_GLOBAL_FUNCTION_REF:
	{
	    uint64_t mci = IDIO_VM_FETCH_REF (thr);
	    IDIO fmci = idio_fixnum (mci);
	    IDIO fgci = idio_module_get_or_set_vci (idio_thread_current_env (), fmci);
	    ...
	    IDIO_VM_RUN_DIS ("GLOBAL-FUNCTION-REF %" PRId64 " %s", mci, IDIO_SYMBOL_S (sym));
		...
		if (idio_S_undef == val) {
		    ...
		    idio_debug ("\nGLOBAL-FUNCTION-REF:) %s", sym);
		    fprintf (stderr, " #%" PRId64, mci);
		    idio_dump (thr, 2);
		    idio_debug ("c-m: %s\n", idio_thread_current_module ());
		    idio_error_printf (IDIO_C_LOCATION ("idio_vm_run1/GLOBAL-FUNCTION-REF"), "unspecified toplevel: %" PRId64 "", mci);
    ...
    case IDIO_A_CHECKED_GLOBAL_FUNCTION_REF:
	{
		...
		idio_debug ("CHECKED-GLOBAL-FUNCTION-REF: %s gvi==0 => sym\n", sym);
		idio_vm_panic (thr, "CHECKED-GLOBAL-FUNCTION-REF: gvi==0");
		IDIO_THREAD_VAL (thr) = sym;
	    }
	}
	break;
    case IDIO_A_CONSTANT_REF:
		...
		break;
	    case IDIO_TYPE_PLACEHOLDER_MARK:
		idio_error_C ("invalid constant type", c, IDIO_C_LOCATION ("idio_vm_run1/CONSTANT-REF"));
	    ...
	    default:
		/* inconceivable! */
		idio_error_printf (IDIO_C_LOCATION ("idio_vm_run1/CONSTANT-REF"), "v=n/k o=%#p o&3=%x F=%x C=%x P=%x", c, (intptr_t) c & IDIO_TYPE_MASK, IDIO_TYPE_FIXNUM_MARK, IDIO_TYPE_CONSTANT_MARK, IDIO_TYPE_POINTER_MARK);
    ...
    case IDIO_A_GLOBAL_DEF:
	{
		...
		sk_ce = IDIO_LIST5 (kind, fmci, fgvi, ce, idio_string_C ("IDIO_A_GLOBAL_DEF"));
		...
		if (0 == IDIO_FIXNUM_VAL (fgvi)) {
		    ...
		    fgvi = idio_fixnum (gvi);
		    idio_module_set_vvi (ce, idio_fixnum (mci), fgvi);
		    sk_ce = IDIO_LIST5 (kind, fmci, fgvi, ce, idio_string_C ("IDIO_A_GLOBAL_DEF/gvi=0"));
		    idio_module_set_symbol (sym, sk_ce, ce);
		}
	    }
	}
    ...
    case IDIO_A_GLOBAL_SET:
	{
		...
		idio_error_runtime_unbound (fmci, fgci, sym, IDIO_C_LOCATION ("idio_vm_run1/GLOBAL-SET"));
    ...
    case IDIO_A_COMPUTED_DEFINE:
	{
	    ...
	    if (idio_S_unspec == sk_ce) {
		sk_ce = IDIO_LIST5 (idio_S_toplevel, idio_fixnum (mci), fgvi, ce, idio_string_C ("IDIO_A_COMPUTED_DEFINE"));
		idio_module_set_symbol (sym, sk_ce, ce);
	    } else {
		IDIO_PAIR_HTT (sk_ce) = fgvi;
	    }
    ...
    case IDIO_A_RETURN:
	{
		...
		idio_vm_decode_stack (thr);
		idio_error_C ("RETURN: not a number", ipc, IDIO_C_LOCATION ("idio_vm_run1/RETURN"));
...
void idio_vm_dasm (IDIO thr, idio_ai_t pc0, idio_ai_t pce)
{
	...
	case IDIO_A_GLOBAL_REF:
		...
		IDIO_VM_DASM ("GLOBAL-REF %" PRId64 "", mci);
	    }
	    break;
	case IDIO_A_CHECKED_GLOBAL_REF:
	    {
		...
		IDIO_VM_DASM ("CHECKED-GLOBAL-REF %" PRId64 "", mci);
	    }
	    break;
	case IDIO_A_GLOBAL_FUNCTION_REF:
	    {
		...
		IDIO_VM_DASM ("GLOBAL-FUNCTION-REF %" PRId64 "", mci);
	    }
	    break;
	case IDIO_A_CHECKED_GLOBAL_FUNCTION_REF:
	    {
		...
		IDIO_VM_DASM ("CHECKED-GLOBAL-FUNCTION-REF %" PRId64 "", mci);
	    }
	    break;
	case IDIO_A_CONSTANT_REF:
	    {
	...
	case IDIO_A_COMPUTED_REF:
		...
		IDIO_VM_DASM ("COMPUTED-REF %" PRId64 "", mci);
	    }
	    break;
	case IDIO_A_PREDEFINED0:
	    {
	...
	case IDIO_A_GLOBAL_DEF:
		...
		if (idio_isa_symbol (sym)) {
		    IDIO_VM_DASM ("GLOBAL-DEF %" PRId64 " %s", mci, IDIO_SYMBOL_S (sym));
		} else {
		    IDIO_VM_DASM ("GLOBAL-DEF %" PRId64 " %s", mci, idio_type2string (sym));
	...
	case IDIO_A_GLOBAL_SET:
		...
		if (idio_isa_symbol (sym)) {
		    IDIO_VM_DASM ("GLOBAL-SET %" PRId64 " %s", mci, IDIO_SYMBOL_S (sym));
		} else {
		    IDIO_VM_DASM ("GLOBAL-SET %" PRId64 " %s", mci, idio_type2string (sym));
		    idio_debug_FILE (idio_dasm_FILE, " %s", sym);
		}
	    }
	    break;
	case IDIO_A_COMPUTED_SET:
		...
		IDIO_VM_DASM ("COMPUTED-SET %" PRId64 "", mci);
	    }
	    break;
	case IDIO_A_COMPUTED_DEFINE:
	    {
		...
		IDIO_VM_DASM ("COMPUTED-DEFINE %" PRId64 "", mci);
	    }
	    break;
	case IDIO_A_LONG_GOTO:
	    {
	...
	case IDIO_A_DYNAMIC_REF:
		...
		IDIO_VM_DASM ("DYNAMIC-REF %" PRId64 "", mci);
	    }
	    break;
	case IDIO_A_DYNAMIC_FUNCTION_REF:
	    {
		...
		IDIO_VM_DASM ("DYNAMIC-FUNCTION-REF %" PRId64 "", mci);
	    }
	    break;
	case IDIO_A_PUSH_ENVIRON:
	    {
	...
	case IDIO_A_ENVIRON_REF:
		...
		IDIO_VM_DASM ("ENVIRON-REF %" PRIu64, mci);
	    }
	    break;
	case IDIO_A_PUSH_TRAP:
	    {
	...
	default:
		...
		fprintf (stderr, "\n");
		idio_error_printf (IDIO_C_LOCATION ("idio_vm_dasm"), "unexpected instruction: %3d @%" PRId64 "\n", ins, pc - 1);
...
IDIO_DEFINE_PRIMITIVE0V ("%%idio-dasm", dasm, (IDIO args))
{
	...
	if (idio_isa_closure (c)) {
	    ...
	    idio_error_param_type ("closure", c, IDIO_C_LOCATION ("%%idio-dasm"));
...
IDIO idio_vm_run (IDIO thr)
{
	...
	if (idio_vm_run1 (thr)) {
		...
		if (idio_command_signal_record[signum]) {
			...
			idio_error_C ("signal without a condition to raise", idio_fixnum (signum), IDIO_C_LOCATION ("idio_vm_run1"));
...
void idio_vm_decode_stack (IDIO thr)
{
    ...
    /* IDIO f_NCE_pc = idio_fixnum (idio_vm_NCE_pc); */
    /* IDIO f_FINISH_pc = idio_fixnum (idio_vm_FINISH_pc); */
    IDIO f_CR_pc = idio_fixnum (idio_vm_CR_pc);
    IDIO f_IHR_pc = idio_fixnum (idio_vm_IHR_pc);

    fprintf (stderr, "vm-decode-stack: thr=%8p esp=%4td dsp=%4td tsp=%4td sp=%4td pc=%6zd\n", thr, esp, dsp, tsp, sp, IDIO_THREAD_PC (thr));
	...
	if (sp < tsp &&
	    0 == seen_tsp &&
	    0 == warn_tsp) {
	    fprintf (stderr, "****\t  missed TSP at %4td: ", tsp);
	    if (tsp >= 1 &&
		sp0 >= 1)  {
		IDIO sv0 = idio_array_get_index (stack, tsp - 0); /* handler */
		IDIO sv1 = idio_array_get_index (stack, tsp - 1); /* condition */
		IDIO sv2 = idio_array_get_index (stack, tsp - 2); /* tsp-next */

		if (idio_isa_procedure (sv0)) {
		    idio_debug ("%s ", sv0);
		} else {
		    idio_debug ("**%s is not a procedure** ", sv0);
		}

		if (idio_isa_fixnum (sv1)) {
		    IDIO fgci = idio_module_get_or_set_vci (idio_thread_current_env (), sv1);
		    idio_debug ("%s ", idio_vm_constants_ref (IDIO_FIXNUM_VAL (fgci)));
		} else {
		    idio_debug ("**%s is not a fixnum** ", sv1);
		}

		fprintf (stderr, "=> ");
		if (idio_isa_fixnum (sv2)) {
		    idio_debug ("%s", sv2);
		} else {
		    idio_debug ("**%s is not a fixnum**", sv2);
		}
	    }
	    fprintf (stderr, "\n");
	    warn_tsp = 1;
	...
	IDIO sv6 = sp >= 6 ? idio_array_get_index (stack, sp - 6) : idio_S_nil;
	IDIO sv7 = sp >= 7 ? idio_array_get_index (stack, sp - 7) : idio_S_nil;
	IDIO sv8 = sp >= 8 ? idio_array_get_index (stack, sp - 8) : idio_S_nil;
	IDIO sv9 = sp >= 9 ? idio_array_get_index (stack, sp - 9) : idio_S_nil;
	...
	if (sp == tsp &&
	    sp >= 2 &&
	    idio_isa_procedure (sv0) &&
	    idio_isa_fixnum (sv1) &&
	    idio_isa_fixnum (sv2)) {
	    fprintf (stderr, "%-20s ", "T/H (tsp)");
	    idio_debug ("%-35s ", sv0);
	    IDIO fgci = idio_module_get_or_set_vci (idio_thread_current_env (), sv1);
	    idio_debug ("%-20s ", idio_vm_constants_ref (IDIO_FIXNUM_VAL (fgci)));
	    tsp = IDIO_FIXNUM_VAL (sv2);
	    fprintf (stderr, "next t/h @%td", tsp);
	    sp -= 3;
	    seen_tsp = 1;
	} else if (sv0 == idio_SM_push_trap &&
		   sp >= 3 &&
	    ...
	    seen_tsp = 1;
		   ...
		   idio_isa_fixnum (sv0) &&
		   idio_isa_fixnum (sv1)) {
	    fprintf (stderr, "%-20s ", "DSP");
	    idio_debug ("#%s ", sv0);
	    idio_debug ("%s", sv1);
	    dsp = IDIO_FIXNUM_VAL (sv1);
	    fprintf (stderr, " => %td", dsp);
	    sp -= 2;
	} else if (sp == esp &&
		   sp >= 1 &&
		   idio_isa_fixnum (sv0) &&
		   idio_isa_fixnum (sv1)) {
	    fprintf (stderr, "%-20s ", "ESP");
	    idio_debug ("#%s ", sv0);
	    idio_debug ("%s", sv1);
	    esp = IDIO_FIXNUM_VAL (sv1);
	    fprintf (stderr, " => %td", esp);
	    sp -= 2;
	} else if (sv0 == f_IHR_pc &&
		   sp >= 9 &&
		   idio_isa_module (sv4) &&
		   (idio_S_nil == sv5 ||
		    idio_isa_frame (sv5)) &&
		   ...
		   idio_isa_fixnum (sv8) &&
		   idio_isa_fixnum (sv9)) {
	    fprintf (stderr, "%-20s ", "IHR");
	    sp -= 10;
	} else if (sv0 == f_CR_pc &&
		   sp >= 6 &&
		   idio_isa_fixnum (sv1) &&
		   idio_isa_procedure (sv2) &&
		   idio_isa_module (sv3) &&
		   (idio_S_nil == sv4 ||
		    idio_isa_frame (sv4)) &&
		   idio_isa_fixnum (sv5) &&
		   idio_isa_fixnum (sv6)) {
	    fprintf (stderr, "%-20s ", "CR");
	    idio_debug ("HSP => %s ", sv1);
	    idio_debug ("esp %s ", sv6);
	    idio_debug ("dsp %s ", sv5);
	    idio_debug ("tsp %s ", sv4);
	    idio_debug ("%s ", sv3);
	    idio_debug ("%s ", sv2);
	    idio_debug ("; PC => %s", sv7);
	    sp -= 7;
	    ...
	    sp -= 2;
		   ...
		   idio_SM_preserve_continuation == sv0 &&
		   idio_isa_fixnum (sv1)) {
