...
static IDIO idio_vm_SYM_IDEF_gvi0_string = idio_S_nil;
static IDIO idio_vm_SYM_SET_string = idio_S_nil;
static IDIO idio_vm_COMPUTED_SYM_DEF_string = idio_S_nil;
static IDIO idio_vm_COMPUTED_SYM_IDEF_string = idio_S_nil;
static IDIO idio_vm_COMPUTED_SYM_IDEF_gvi0_string = idio_S_nil;
static IDIO idio_vm_EXPANDER_string = idio_S_nil;
static IDIO idio_vm_IEXPANDER_string = idio_S_nil;
...
idio_xenv_t *idio_xenv ()
{
	...
	IDIO_XENV_SRC_PROPS (xenv)      = idio_vm_src_props;
...
idio_xi_t idio_new_xenv (IDIO desc)
{
    ...
    IDIO_XENV_SYMBOLS (xenv)        = idio_array (0);
    IDIO_XENV_CONSTANTS (xenv)      = idio_array (0);
    IDIO_XENV_CONSTANTS_HASH (xenv) = IDIO_HASH_EQP (8);
    IDIO_XENV_SRC_EXPRS (xenv)      = idio_array (0);
    IDIO_XENV_SRC_PROPS (xenv)      = idio_array (0);
    IDIO_XENV_VALUES (xenv)         = idio_array (0);

    idio_gc_protect_auto (IDIO_XENV_DESC (xenv));
    idio_gc_protect_auto (IDIO_XENV_SYMBOLS (xenv));
    idio_gc_protect_auto (IDIO_XENV_CONSTANTS (xenv));
    idio_gc_protect_auto (IDIO_XENV_CONSTANTS_HASH (xenv));
    idio_gc_protect_auto (IDIO_XENV_SRC_EXPRS (xenv));
    idio_gc_protect_auto (IDIO_XENV_SRC_PROPS (xenv));
    idio_gc_protect_auto (IDIO_XENV_VALUES (xenv));
...
void idio_vm_dynamic_set (IDIO thr, idio_as_t mci, idio_as_t gvi, IDIO v)
{
	...
	if (sp >= 3) {
	     ...
	     * be in xi==0
	     */
	    IDIO vs = IDIO_XENV_VALUES (idio_xenvs[0]);
...
IDIO idio_vm_computed_ref (idio_xi_t xi, idio_as_t mci, idio_as_t vi)
{
	...
	if (idio_isa_primitive (get) ||
	     ...
	     * cvar-so
	     */
	    idio_vm_error_computed_no_accessor ("get", mci, vi, IDIO_C_FUNC_LOCATION ());
	 ...
	 * Coding error.
	 */
	idio_vm_error_computed ("no get/set accessors", mci, vi, IDIO_C_FUNC_LOCATION ());
...
IDIO idio_vm_computed_set (idio_xi_t xi, idio_as_t mci, idio_as_t vi, IDIO v)
{
    IDIO_ASSERT (v);
	...
	if (idio_isa_primitive (set) ||
	     ...
	     * cvar-go = 3
	     */
	    idio_vm_error_computed_no_accessor ("set", mci, vi, IDIO_C_FUNC_LOCATION ());
...
void idio_vm_computed_define (idio_xi_t xi, idio_as_t mci, idio_as_t gvi, IDIO v)
{
    IDIO_ASSERT (v);
...
void idio_vm_raise_condition (IDIO continuablep, IDIO condition, int IHR, int reraise)
{
	...
	IDIO trap_ct_sym = idio_vm_constants_ref (0, (idio_ai_t) IDIO_FIXNUM_VAL (trap_ct_mci));
...
static idio_as_t idio_vm_get_or_create_vvi (IDIO thr, idio_as_t mci)
{
	...
	if (idio_S_false == si_ce) {
		...
		idio_module_set_symbol_value_xi (xi, sym, sym, ce);
...
IDIO idio_vm_iref2val (IDIO thr, idio_xi_t xi, idio_as_t si, char *const op)
{
	...
	if (idio_S_false == si_ce) {
	    IDIO mdr = idio_module_direct_reference (sym);

	    if (idio_S_false == mdr) {
		/*
		 * Squirrely bootstrap.  "import object" calls
		 * a primitive in object but the import hasn't
		 * completed and so object is not on the list
		 * of imports for
		 * idio_module_find_symbol_recurse(), above.
		 */
		IDIO si_mod = idio_module_find_symbol_recurse (sym, IDIO_THREAD_MODULE (thr), 1);

		if (idio_S_false == si_mod) {
		    /*
		     * This happens for:
		     *
		     * * first use of a name:
		     ...
		     *     been defined (ans: no)
		     *
		     * Leaving no easy way to identify a genuine
		     * failure.  Which is annoying.
		     */
		    fprintf (stderr, "%s: NOTICE: lookup failed for ", op);
		    idio_debug ("%s ", sym);
		    idio_debug ("in %s ", ce);
		    idio_debug ("in %s -- will become a symbol\n", IDIO_THREAD_MODULE (thr));
		} else {
		    info = "m";	/* module lookup */

		    fgvi = IDIO_SI_VI (si_mod);
		    gvi = IDIO_FIXNUM_VAL (fgvi);
		    si_ce = si_mod; /* for printing, below */
		}
		/*
		 * If we failed to look it up (by hook or by crook)
		 * we'll now fall through to the missing clause
		 */
	    } else {
		info = "d"; /* direct reference */

		/* (mod sym si) */
		si_ce = IDIO_PAIR_HTT (mdr);
		...
		if (0 == gvi) {
		     ...
		     * Coding error.
		     */
		    idio_error_runtime_unbound (fsi, fgvi, sym, IDIO_C_FUNC_LOCATION_S (op));
	...
	/*
	fprintf (stderr, "%-17s %s [%zu].%-4zu => [0].%-4zu", op, info, xi, si, gvi);
	idio_debug (" %-20s ", sym);
	idio_debug (" %-20s ", ce);
	idio_debug ("%s\n", si_ce);
	*/
    ...
    if (2602 == gvi) {
	fprintf (stderr, "%s:   [%zu].%-4zu => [0].%-4zu", op, xi, si, gvi);
	idio_debug (" %s\n", val);
    }
	 ...
	 * Test Case: ??
	 *
	 * It means that the VM's global table of values has undefined
	 * values in it.  Which can't be a good thing.
	 */
	idio_error_runtime_unbound (fsi, fgvi, sym, IDIO_C_FUNC_LOCATION_S (op));
...
int idio_vm_run1 (IDIO thr)
{
    ...
    case IDIO_A_SYM_IREF:
	{
	    ...
	    IDIO_VM_RUN_DIS ("%-17s   .%-4" PRIu64 " %-20s ", "SYM-IREF", si, IDIO_SYMBOL_S (sym));
#else
	    IDIO_VM_RUN_DIS ("%-17s   .%-4" PRIu64 " ", "SYM-IREF", si);
#endif

	    IDIO_THREAD_VAL (thr) = idio_vm_iref2val (thr, xi, si, "SYM-IREF");
	}
	break;
    ...
    case IDIO_A_FUNCTION_SYM_IREF:
	{
	    ...
	    IDIO_THREAD_VAL (thr) = idio_vm_iref2val (thr, xi, si, "FUNCTION-SYM-IREF");
	}
	break;
    ...
    case IDIO_A_CONSTANT_IREF:
	{
	    ...
	    IDIO_VM_RUN_DIS ("%-17s   .%" PRIu64 " ", "CONSTANT-IREF", ci);
#ifdef IDIO_VM_DIS
	    if (idio_vm_dis) {
		 ...
		 * Coding error.
		 */
		idio_coding_error_C ("invalid constant type", c, IDIO_C_FUNC_LOCATION_S ("CONSTANT-IREF"));
	    ...
	    case IDIO_TYPE_POINTER_MARK:
		{
		    ...
		    case IDIO_TYPE_PRIMITIVE:
		    case IDIO_TYPE_CLOSURE:
			idio_debug ("idio_vm_run1/CONSTANT-IREF: you should NOT be reifying %s", c);
			IDIO name = idio_ref_property (c, idio_KW_name, idio_S_unspec);
			if (idio_S_unspec != name) {
			    idio_debug (" %s", name);
			}
	    ...
	    default:
		/* inconceivable! */
		idio_error_printf (IDIO_C_FUNC_LOCATION_S ("CONSTANT-IREF"), "v=n/k o=%#p o&3=%x F=%x C=%x P=%x", c, (intptr_t) c & IDIO_TYPE_MASK, IDIO_TYPE_FIXNUM_MARK, IDIO_TYPE_CONSTANT_MARK, IDIO_TYPE_POINTER_MARK);
    ...
    case IDIO_A_COMPUTED_SYM_IREF:
	{
	    ...
	    IDIO_VM_RUN_DIS ("%-17s   .%-4" PRIu64 " %-20s ", "COMPUTED-SYM-IREF", si, IDIO_SYMBOL_S (sym));
#else
	    IDIO_VM_RUN_DIS ("%-17s   .%-4" PRIu64 " ", "COMPUTED-SYM-IREF", si);
#endif

	    IDIO gns = idio_vm_iref2val (thr, xi, si, "COMPUTED-SYM-IREF");
    ...
    case IDIO_A_SYM_IDEF:
	{
		...
		fprintf (stderr, "SYM-IDEF: [%zu].%zu > %zu\n", xi, si, idio_array_size (st));
		IDIO_C_ASSERT (0);
	    }
	    ...
	    IDIO_VM_RUN_DIS ("%-17s   .%-4" PRIu64 " %-20s ", "SYM-IDEF", si, IDIO_SYMBOL_S (sym));
		...
		si_ce = IDIO_LIST6 (kind, fsi, fgci, fgvi, ce, idio_vm_SYM_IDEF_string);
		idio_module_set_symbol (sym, si_ce, ce);
	    } else {
		fgvi = IDIO_SI_VI (si_ce);
		if (0 == IDIO_FIXNUM_VAL (fgvi)) {
		    ...
		    si_ce = IDIO_LIST6 (kind,
					fsi,
					IDIO_SI_CI (si_ce),
					fgvi,
					ce,
					idio_vm_SYM_IDEF_gvi0_string);
		    idio_module_set_symbol (sym, si_ce, ce);
		}
	    }
	    ...
	    /*
	    fprintf (stderr, "%-17s   [%zu].%-4zu => [0].%-4zu ", "SYM-IDEF", xi, si, gvi);
	    idio_debug ("%s/", IDIO_MODULE_NAME (ce));
	    idio_debug ("%s ", sym);
    ...
    case IDIO_A_SYM_ISET:
	{
	    ...
	    IDIO_VM_RUN_DIS ("%-17s   .%-4" PRIu64 " [0].%" PRId64 " %-20s ", "SYM-ISET", si, gvi, IDIO_SYMBOL_S (sym));

	    if (gvi) {
		if (2602 == gvi) {
		    fprintf (stderr, "SYM-ISET:   [%zu].%-4zu => [0].%-4zu ", xi, si, gvi);
		    idio_debug ("%-20s", sym);
		    idio_debug ("%s\n", IDIO_THREAD_VAL (thr));
		}
		...
		if (idio_S_false != si_ce &&
		    ...
		    ! idio_isa_primitive (IDIO_THREAD_VAL (thr))) {
		    idio_debug ("SYM-ISET %s -> toplevel", IDIO_SI_SCOPE (si_ce));
		    idio_debug (" for %-20s", sym);
		    idio_debug (" == %s\n", IDIO_THREAD_VAL (thr));
		...
		IDIO ce = idio_thread_current_env ();
		IDIO si_ce = idio_module_find_symbol (sym, ce);
		idio_debug ("SYM-ISET: UNBOUND sym=%s", sym);
		idio_debug (" ce=%s\n", IDIO_MODULE_NAME (ce));
		idio_debug (" si_ce=%s\n", si_ce);
		idio_debug (" MI=%s\n", IDIO_MODULE_IMPORTS (ce));

		IDIO sym_si = idio_module_find_symbol_recurse (sym, ce, 1);
		idio_debug (" si=%s\n", sym_si);

		idio_error_runtime_unbound (idio_fixnum (si), fgvi, sym, IDIO_C_FUNC_LOCATION_S ("SYM-ISET"));
		idio_vm_panic (thr, "SYM-ISET: no gvi!");
    ...
    case IDIO_A_COMPUTED_SYM_ISET:
	{
	    ...
	    IDIO_VM_RUN_DIS ("%-17s   .%-4" PRIu64 " %-20s ", "COMPUTED-SYM-ISET", si, IDIO_SYMBOL_S (sym));
#else
	    IDIO_VM_RUN_DIS ("%-17s   .%-4" PRIu64 " ", "COMPUTED-SYM-ISET", si);
#endif

	    IDIO gns = idio_vm_iref2val (thr, xi, si, "COMPUTED-SYM-ISET");
	    IDIO val = IDIO_THREAD_VAL (thr);
    ...
    case IDIO_A_COMPUTED_SYM_DEF:
	{
	    ...
	    idio_ai_t gvi = idio_vm_extend_values (xi);
	    IDIO fgvi = idio_fixnum (gvi);
	    idio_module_set_vvi (ce, idio_fixnum (mci), fgvi);
    ...
    case IDIO_A_COMPUTED_SYM_IDEF:
	{
	    ...
	    IDIO_VM_RUN_DIS ("%-17s   .%-4" PRIu64 " %-20s", "COMPUTED-SYM-IDEF", si, IDIO_SYMBOL_S (sym));
		...
		si_ce = IDIO_LIST6 (idio_S_toplevel, fsi, fgci, fgvi, ce, idio_vm_COMPUTED_SYM_IDEF_string);
		idio_module_set_symbol (sym, si_ce, ce);
	    } else {
		fgvi = IDIO_SI_VI (si_ce);
		if (0 == IDIO_FIXNUM_VAL (fgvi)) {
		    ...
		    si_ce = IDIO_LIST6 (idio_S_toplevel,
					...
					fgvi,
					ce,
					idio_vm_COMPUTED_SYM_IDEF_gvi0_string);
		    idio_module_set_symbol (sym, si_ce, ce);
		}
	    }
	    ...
	    fprintf (stderr, "%-17s   [%zu].%-4zu => [0].%-4zu ", "COMPUTED-SYM-IDEF", xi, si, gvi);
	    idio_debug ("%s\n", sym);
	    IDIO vs = IDIO_XENV_VALUES (idio_xenvs[xi]);
	    idio_array_set (vs, fsi, fgvi);
    ...
    case IDIO_A_VAL_SET:
	{
	    ...
	    IDIO_VM_RUN_DIS ("%-17s   %" PRIu64, "VAL-SET", gvi);
    ...
    case IDIO_A_VAL_ISET:
	{
	    ...
	    IDIO_VM_RUN_DIS ("%-17s   .%-4" PRIu64 " ", "VAL-ISET", vi);
		...
		if (0 == gvi) {
		    ...
		    /* fprintf (stderr, "%-17s s [%zu].%-4zu => [0].%-4zu\n", "VAL-ISET", xi, vi, gvi); */
		    idio_vm_values_set (xi, vi, fgvi);
		}

		IDIO_VM_RUN_DIS ("[0].%" PRIu64 " ", gvi);

		/* fprintf (stderr, "%-17s r [%zu].%-4zu => [0].%-4zu\n", "VAL-ISET", xi, vi, gvi); */
		idio_vm_values_set (0, gvi, IDIO_THREAD_VAL (thr));
	    } else {
		idio_vm_panic (thr, "VAL-ISET: no vi!");
	    }
	}
	break;
    ...
    case IDIO_A_PREDEFINED:
	{
	    ...
	    IDIO_VM_RUN_DIS ("%-17s   %" PRIu64 " %-20s", "PREDEFINED", vi, IDIO_PRIMITIVE_NAME (pd));
	...
	{
	    uint64_t vi = idio_vm_fetch_varuint (bc, thr);
	    IDIO_VM_RUN_DIS ("%-17s   .%-4" PRIu64 " ", "IPREDEFINED", vi);
	    IDIO pd = idio_vm_iref2val (thr, xi, vi, "IPREDEFINED");
	    if (idio_isa_primitive (pd)) {
		IDIO_VM_RUN_DIS ("%-20s", IDIO_PRIMITIVE_NAME (pd));
	    } else {
		IDIO_VM_RUN_DIS ("%-20s", idio_type2string (pd));
    ...
    case IDIO_A_CREATE_IFUNCTION:
	{
	    ...
	    IDIO_VM_RUN_DIS ("CREATE-IFUNCTION @ +%" PRIu64 " %-20s ", i, IDIO_SYMBOL_S (name));
	...
	{
	    uint64_t gvi = IDIO_VM_FETCH_REF (thr, bc);
	    IDIO_VM_RUN_DIS ("CREATE-CLOSURE %" PRIu64, gvi);

	    IDIO_THREAD_VAL (thr) = idio_closure (idio_vm_values_ref (xi, gvi),
						  IDIO_THREAD_FRAME (thr));
	}
	break;
	...
	{
	    uint64_t vi = IDIO_VM_FETCH_REF (thr, bc);
	    IDIO_VM_RUN_DIS ("CREATE-ICLOSURE .%-4" PRIu64 " ", vi);

	    IDIO cl = idio_vm_values_gref (xi, vi, "CREATE-ICLOSURE");
    ...
    case IDIO_A_LINK_IFRAME:
	{
	    ...
	    IDIO_VM_RUN_DIS ("LINK-IFRAME si=%" PRIu64, si);
    ...
    case IDIO_A_EXTEND_IFRAME:
	{
	    uint64_t alloc = idio_vm_fetch_varuint (bc, thr);
	    uint64_t si = idio_vm_fetch_varuint (bc, thr);
	    IDIO_VM_RUN_DIS ("EXTEND-IFRAME %" PRIu64 " sci=%" PRIu64, alloc, si);
    ...
    case IDIO_A_PRIMCALL0:
	{
	    ...
	    IDIO_VM_RUN_DIS ("PRIMITIVE/0 %" PRIu64 " %s", vi, IDIO_PRIMITIVE_NAME (primdata));
    ...
    case IDIO_A_IPRIMCALL0:
	{
	    ...
	    IDIO pd = idio_vm_iref2val (thr, xi, si, "IPRIMITIVE/0");
	    IDIO_TYPE_ASSERT (primitive, pd);
	    /*
	     * XXX see comment in idio_meaning_primitive_application()
	     * for setting *func*
	    ...
	    IDIO_VM_RUN_DIS ("IPRIMITIVE/0 .%-4" PRIu64 " %s", si, IDIO_PRIMITIVE_NAME (pd));
    ...
    case IDIO_A_PRIMCALL1:
	{
	     ...
	     */
	    IDIO_THREAD_FUNC (thr) = primdata;
	    IDIO_VM_RUN_DIS ("PRIMITIVE/1 %" PRIu64 " %s", vi, IDIO_PRIMITIVE_NAME (primdata));
    ...
    case IDIO_A_IPRIMCALL1:
	{
	    ...
	    IDIO pd = idio_vm_iref2val (thr, xi, si, "IPRIMITIVE/1");
	    IDIO_TYPE_ASSERT (primitive, pd);
	    /*
	     * XXX see comment in idio_meaning_primitive_application()
	     * for setting *func*
	     */
	    IDIO_THREAD_FUNC (thr) = pd;
	    IDIO_VM_RUN_DIS ("IPRIMITIVE/1 .%-4" PRIu64 " %s", si, IDIO_PRIMITIVE_NAME (pd));
	    idio_vm_primitive_call_trace (pd, thr, 0);
#ifdef IDIO_VM_PROF
	    struct timespec prim_t0;
	    struct rusage prim_ru0;
    ...
    case IDIO_A_PRIMCALL2:
	{
	     ...
	     */
	    IDIO_THREAD_FUNC (thr) = primdata;
	    IDIO_VM_RUN_DIS ("PRIMITIVE/2 %" PRIu64 " %s", vi, IDIO_PRIMITIVE_NAME (primdata));
    ...
    case IDIO_A_IPRIMCALL2:
	{
	    ...
	    IDIO pd = idio_vm_iref2val (thr, xi, si, "IPRIMITIVE/2");
	    IDIO_TYPE_ASSERT (primitive, pd);
	    /*
	     * XXX see comment in idio_meaning_primitive_application()
	     * for setting *func*
	     */
	    IDIO_THREAD_FUNC (thr) = pd;
	    IDIO_VM_RUN_DIS ("IPRIMITIVE/2 .%-4" PRIu64 " %s", si, IDIO_PRIMITIVE_NAME (pd));
	    idio_vm_primitive_call_trace (pd, thr, 0);
#ifdef IDIO_VM_PROF
	    struct timespec prim_t0;
	    struct rusage prim_ru0;
	...
	{
	    uint64_t si = IDIO_VM_FETCH_REF (thr, bc);
	    IDIO_VM_RUN_DIS ("IEXPANDER .%-4" PRIu64 " ", si);
		...
		if (idio_S_false == si_ce) {
		    ...
		    si_ce = IDIO_LIST6 (idio_S_toplevel, fsi, fci, fgvi, ce, idio_vm_IEXPANDER_string);
		    idio_module_set_symbol (sym, si_ce, ce);
		} else {
		    fgvi = IDIO_SI_VI (si_ce);
		    gvi = IDIO_FIXNUM_VAL (fgvi);
    ...
    case IDIO_A_INFIX_IOPERATOR:
	{
	    ...
	    IDIO_VM_RUN_DIS ("INFIX-IOPERATOR .%-4" PRIu64 " pri %4" PRIu64 " ", si, pri);
		...
		if (idio_S_false == si_op) {
		    idio_debug ("INFIX-IOPERATOR: %-20s ", sym);
		    fprintf (stderr, ".%-4" PRIu64 " undefined?  setting...\n", si);
		    ...
		    si_op = IDIO_LIST6 (idio_S_toplevel, fsi, fci, fgvi, idio_operator_module, idio_vm_INFIX_IOPERATOR_string);
    ...
    case IDIO_A_POSTFIX_IOPERATOR:
	{
	    ...
	    IDIO_VM_RUN_DIS ("POSTFIX-IOPERATOR .%-4" PRIu64 " pri %4" PRIu64 " ", si, pri);
		...
		if (idio_S_false == si_op) {
		    idio_debug ("POSTFIX-IOPERATOR: %-20s ", sym);
		    fprintf (stderr, ".%-4" PRIu64 " undefined?  setting...\n", si);
		    ...
		    si_op = IDIO_LIST6 (idio_S_toplevel, fsi, fci, fgvi, idio_operator_module, idio_vm_POSTFIX_IOPERATOR_string);
		    idio_module_set_symbol (sym, si_op, idio_operator_module);
		}
	...
	{
	    uint64_t mci = IDIO_VM_FETCH_REF (thr, bc);
	    IDIO_VM_RUN_DIS ("PUSH-DYNAMIC %" PRIu64, mci);
	    idio_ai_t gvi = idio_vm_get_or_create_vvi (thr, mci);
	...
	{
	    uint64_t mci = IDIO_VM_FETCH_REF (thr, bc);
	    IDIO_VM_RUN_DIS ("DYNAMIC-SYM-REF %" PRIu64, mci);
	    idio_ai_t gvi = idio_vm_get_or_create_vvi (thr, mci);
    ...
    case IDIO_A_DYNAMIC_SYM_IREF:
	{
	    ...
	    IDIO_VM_RUN_DIS ("DYNAMIC-SYM-IREF .%-4" PRIu64 " %-20s ", si, IDIO_SYMBOL_S (sym));
		...
		if (idio_S_false == si_ce) {
		     ...
		     * Coding error.
		     */
		    idio_error_runtime_unbound (fsi, fgvi, sym, IDIO_C_FUNC_LOCATION_S ("DYNAMIC-SYM-IREF"));
		...
		/* fprintf (stderr, "DYNAMIC-SYM-IREF s [%zu].%-4zu => [0].%-4zu\n", xi, si, gvi); */
		idio_array_set (vs, fsi, fgvi);
	    }

	    IDIO_VM_RUN_DIS ("[0].%" PRIu64 " ", gvi);

	    /* fprintf (stderr, "DYNAMIC-SYM-IREF r [%zu].%-4zu => [0].%-4zu ", xi, si, gvi); */
	    /* idio_debug ("%s\n", sym); */
	...
	{
	    uint64_t mci = IDIO_VM_FETCH_REF (thr, bc);
	    IDIO_VM_RUN_DIS ("DYNAMIC-FUNCTION-SYM-REF %" PRIu64, mci);
	    idio_ai_t gvi = idio_vm_get_or_create_vvi (thr, mci);
	...
	{
	    uint64_t mci = IDIO_VM_FETCH_REF (thr, bc);
	    IDIO_VM_RUN_DIS ("PUSH-ENVIRON %" PRIu64, mci);
	    idio_ai_t gvi = idio_vm_get_or_create_vvi (thr, mci);
	...
	{
	    uint64_t mci = IDIO_VM_FETCH_REF (thr, bc);
	    IDIO_VM_RUN_DIS ("ENVIRON-SYM-REF %" PRIu64, mci);
	    idio_ai_t gvi = idio_vm_get_or_create_vvi (thr, mci);
    ...
    case IDIO_A_ENVIRON_SYM_IREF:
	{
	    ...
	    IDIO_VM_RUN_DIS ("ENVIRON-SYM-IREF .%-4" PRIu64 " %-20s ", si, IDIO_SYMBOL_S (sym));
		...
		if (idio_S_false == si_ce) {
		     ...
		     * Coding error.
		     */
		    idio_error_runtime_unbound (fsi, fgvi, sym, IDIO_C_FUNC_LOCATION_S ("ENVIRON-SYM-IREF"));
		...
		/* fprintf (stderr, "ENVIRON-SYM-IREF s [%zu].%-4zu => [0].%-4zu\n", xi, si, gvi); */
		idio_array_set (vs, fsi, fgvi);
	    }

	    IDIO_VM_RUN_DIS ("[0].%" PRIu64 " ", gvi);

	    /* fprintf (stderr, "ENVIRON-SYM-IREF r [%zu].%-4zu => [0].%-4zu ", xi, si, gvi); */
	    /* idio_debug ("%s\n", sym); */
    ...
    case IDIO_A_PUSH_TRAP:
	{
	    ...
	    IDIO_VM_RUN_DIS ("PUSH-TRAP %" PRIu64, mci);
    ...
    case IDIO_A_PUSH_ITRAP:
	{
	    ...
	    IDIO_VM_RUN_DIS ("PUSH-ITRAP %" PRIu64, si);

	    /*
	     * The "value" is the gci for this symbol, ie. the global
	     * constant index (not our execution environment constant
	     * index).
	     */
	    IDIO vs = IDIO_XENV_VALUES (idio_xenvs[xi]);
	    IDIO fgci = idio_array_ref_index (vs, si);
	    idio_ai_t gci = IDIO_FIXNUM_VAL (fgci);
    ...
    case IDIO_A_PUSH_ESCAPER:
	{
	    ...
	    IDIO_VM_RUN_DIS ("PUSH-ESCAPER %" PRIu64, mci);
    ...
    case IDIO_A_ESCAPER_LABEL_REF:
	{
	    ...
	    IDIO_VM_RUN_DIS ("ESCAPER-LABEL_REF %" PRIu64, mci);
...
IDIO idio_vm_run (IDIO thr, idio_pc_t pc, idio_vm_run_enum caller)
{
	...
	if (IDIO_THREAD_PC (thr) != (idio_vm_FINISH_pc + 1)) {
	    fprintf (stderr, "vm-run: THREAD #%d [%zu]@%zd failed to run to FINISH: PC %zd != %zd\n",
...
")
{
    ...
    IDIO_THREAD_XI (thr) = C_xi;
    IDIO r = idio_vm_run (thr, pc, IDIO_VM_RUN_IDIO);
...
IDIO idio_vm_symbols_ref (idio_xi_t xi, idio_as_t si)
{
    ...
    if (si >= idio_array_size (st)) {
	fprintf (stderr, "vm-symbols-ref: [%zu].%zu > %zu\n", xi, si, idio_array_size (st));
	idio_vm_dump_all ();
	IDIO_C_ASSERT (0);
    }

    IDIO fci = idio_array_ref_index (st, si);
    assert (idio_isa_fixnum (fci));
...
idio_as_t idio_vm_extend_constants (idio_xi_t xi, IDIO v)
{
    ...
    IDIO cs = IDIO_XENV_CONSTANTS (idio_xenvs[xi]);
    IDIO ch = IDIO_XENV_CONSTANTS_HASH (idio_xenvs[xi]);
...
IDIO idio_vm_constants_ref (idio_xi_t xi, idio_as_t ci)
{
    ...
    if (ci >= idio_array_size (cs)) {
	fprintf (stderr, "vm-constants-ref: [%zu].%zu > %zu\n", xi, ci, idio_array_size (cs));
	idio_vm_dump_all ();
	IDIO_C_ASSERT (0);
    }
...
IDIO idio_vm_src_expr_ref (idio_xi_t xi, idio_as_t sei)
{
	...
	/* fprintf (stderr, "vm-src-exprs-ref: [%zu].%zu > %zu\n", xi, sei, idio_array_size (ses)); */
	return idio_S_false;
	IDIO_C_ASSERT (0);
    }
...
IDIO idio_vm_src_props_ref (idio_xi_t xi, idio_as_t spi)
{
	...
	/* fprintf (stderr, "vm-src-props-ref [%zu].%zu > %zu\n", xi, spi, idio_array_size (sps)); */
	return idio_S_false;
	IDIO_C_ASSERT (0);
    }
...
IDIO idio_vm_values_ref (idio_xi_t xi, idio_as_t vi)
{
    if (vi) {
	IDIO vs = IDIO_XENV_VALUES (idio_xenvs[xi]);
	if (vi > idio_array_size (vs)) {
	    fprintf (stderr, "vm-values-ref [%zu].%zu > vs %zu\n", xi, vi, idio_array_size (vs));
	    IDIO st = IDIO_XENV_SYMBOLS (idio_xenvs[xi]);
	    if (st &&
		idio_isa_array (st)) {
...
{
    if (xi) {
	IDIO gvi = idio_vm_values_ref (xi, vi);
	fprintf (stderr, "vmv-gset [%zu].%-4zu => ", xi, vi);
	IDIO_TYPE_ASSERT (fixnum, gvi);
	vi = IDIO_FIXNUM_VAL (gvi);
	fprintf (stderr, "[0].%-4zu\n", vi);

	xi = 0;
    }

    idio_vm_values_set (xi, vi, v);
}
...
void idio_vm_thread_state (IDIO thr)
{
	...
	IDIO cs = IDIO_XENV_CONSTANTS (idio_xenvs[frame_xi]);
...
void idio_vm_trap_state (IDIO thr)
{
	...
	while (1) {
	    ...
	    idio_ai_t mci = IDIO_FIXNUM_VAL (ct_mci);

	    IDIO ct_sym = idio_vm_constants_ref (0, mci);
...
#endif
    r = idio_vm_run (thr, pc, IDIO_VM_RUN_C);
...
idio_xi_t idio_vm_add_xenv (IDIO desc, IDIO symbols, IDIO constants, IDIO values, IDIO src_exprs, IDIO src_props, IDIO bc)
{
    IDIO_ASSERT (desc);
    IDIO_ASSERT (symbols);
     ...
     */
    IDIO_XENV_DESC (xe)		  = desc; /* idio_copy (desc, IDIO_COPY_SHALLOW); */
    idio_gc_protect_auto (IDIO_XENV_DESC (xe));

    IDIO_XENV_SYMBOLS (xe)        = symbols; /* idio_copy (symbols, IDIO_COPY_SHALLOW); */
    idio_gc_protect_auto (IDIO_XENV_SYMBOLS (xe));

    IDIO_XENV_CONSTANTS (xe)      = constants; /* idio_copy (constants, IDIO_COPY_SHALLOW); */
    idio_gc_protect_auto (IDIO_XENV_CONSTANTS (xe));

    IDIO_XENV_VALUES (xe)         = values; /* idio_copy (values, IDIO_COPY_SHALLOW); */
    idio_gc_protect_auto (IDIO_XENV_VALUES (xe));

    IDIO_XENV_SRC_EXPRS (xe)      = src_exprs; /* idio_copy (src_exprs, IDIO_COPY_SHALLOW); */
    idio_gc_protect_auto (IDIO_XENV_SRC_EXPRS (xe));

    IDIO_XENV_SRC_PROPS (xe)      = src_props; /* idio_copy (src_props, IDIO_COPY_SHALLOW); */
     ...
     */
    IDIO_XENV_CONSTANTS_HASH (xe) = IDIO_HASH_EQP (al ? al : 1);
	...
	IDIO_IA_REFCNT (ia)++;
	fprintf (stderr, "add-xenv bc rc %d us %zd\n", IDIO_IA_REFCNT (ia), IDIO_IA_USIZE (ia));
    } else {
...
idio_xi_t idio_vm_add_xenv_from_eenv (IDIO thr, IDIO eenv)
{
    ...
    /*
     * Depending on what the code looks like we might have no symbols
     * or no values or the one might be larger than the other.
     * (Generally, values would be larger than symbols but we're in
     * the margins, here.)
     */
    ...
    idio_as_t st_len = idio_list_length (st);

    idio_as_t alen = vs_len;
    if (st_len > alen) {
	alen = st_len;
    }
    if (0 == alen) {
    ...
    /*
     * values has entries defaulting to 0 to be resolved at runtime
     * into a gvi (aka the values entry for xi==0)
     */
    IDIO values = idio_array_dv (alen, idio_fixnum (0));
	...
	if (idio_isa_pair (si)) {
	    idio_array_insert_index (symbols,
				     IDIO_SI_CI (si),
     ...
     * no official API
     */
    IDIO_ARRAY_USIZE (values) = alen;
    IDIO_ARRAY_USIZE (symbols) = alen;

    IDIO constants = idio_struct_instance_ref_direct (eenv, IDIO_EENV_ST_CONSTANTS);

    IDIO src_exprs = idio_struct_instance_ref_direct (eenv, IDIO_EENV_ST_SRC_EXPRS);

    IDIO src_props = idio_struct_instance_ref_direct (eenv, IDIO_EENV_ST_SRC_PROPS);
...
#define IDIO_VM_STRING(c,s) idio_vm_ ## c ## _string = idio_string_C (s); idio_gc_protect_auto (idio_vm_ ## c ## _string);
    ...
    IDIO_VM_STRING (SYM_IDEF_gvi0,         "SYM-IDEF/gvi=0");
    IDIO_VM_STRING (SYM_SET,               "SYM-SET");
    IDIO_VM_STRING (COMPUTED_SYM_DEF,      "COMPUTED-SYM-DEF");
    IDIO_VM_STRING (COMPUTED_SYM_IDEF,     "COMPUTED-SYM-IDEF");
    IDIO_VM_STRING (COMPUTED_SYM_IDEF_gvi0, "COMPUTED-SYM-IDEF/gvi=0");
    IDIO_VM_STRING (EXPANDER,              "EXPANDER");
    IDIO_VM_STRING (IEXPANDER,             "IEXPANDER");
