...
static void idio_read_error (IDIO handle, IDIO lo, IDIO c_location, IDIO msg)
{
    ...
    IDIO c = idio_struct_instance (idio_condition_read_error_type,
				   IDIO_LIST5 (msg,
					       idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_NAME),
					       detail,
					       idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_LINE),
					       idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_POS)));
...
idio_unicode_t idio_read_character_int (IDIO handle, IDIO lo, int kind)
{
    ...
    /*
     * If this is called from outside read.c then lo is idio_S_nil.
     *
     * Normally, not a problem but it turns out that when reading in
     * an octet string (hint: byte code in pre-compiled files) we call
     * idio_getc_handle() from idio_read_uintmax_radix() because we
     * read in \xHH.
     *
     * So, when we previously called
     *
     *   lo = idio_read_lexobj_from_handle (handle);
     *
     * we were, unhelpfully, making an awful lot of calls to
     * idio_struct_instance() and idio_listv().
     *
	...
	if (EOF == ucv) {
		 ...
		 * #\x
		 *
		 * 1) where x is a literal byte which *is* a UTF-8
		 * prefix
		 *
		 ...
		 * prefix of some sort (as you'll get EOF, above,
		 * instead when this goes round the loop).  0xFE and
		 * 0xFF cannot appear in a valid UTF-8 sequence --
		 * remember UTF-8 != Unicode.
		 *
		 * For character-invalid-utf8-eof, the test uses a
		 * literal 0xFE byte which, depending on the mood/whim
		 * of your editor, may appear as LATIN SMALL LETTER
		 * THORN from C1 Controls and Latin-1 Supplement.
		 * Emacs and vi seem to guess at ISO 8859-1 whereas
		 * less displays <FE>.
		 *
		 * Also be leery of cut'n'paste as your GUI may do the
		 * decent thing and convert the, Unicode code point
		 * U+00FE LATIN SMALL LETTER THORN into a UTF-8 0xC3
		 * 0xBE sequence which is correctly 0xFE in Unicode
		 * thus defeating the point of our invalid UTF-8 test.
		 *
		 * *shakes fist*
		 */
	...
	if (IDIO_UTF8_ACCEPT == state) {
	     ...
	     * #\xy
	     *
	     * where x is a literal byte which *is* a UTF-8 prefix and
	     * y is not a suitable following byte
	     *
...
static IDIO idio_read_list (IDIO handle, IDIO list_lo, IDIO opendel, idio_unicode_t ic[IDIO_TEMPLATE_IC], int depth)
{
	...
	IDIO e = idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_EXPR);
	...
	if (idio_eofp_handle (handle)) {
	     ...
	     */
	    IDIO lo = idio_read_1_expr (handle, ic, depth);
	    IDIO pt = idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_EXPR);
	    /*
	     * Careful of:
	     *
	     * ( a &
	    ...
	    while (idio_T_eol == pt) {
		IDIO lo = idio_read_1_expr (handle, ic, depth);
		pt = idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_EXPR);
	    }
	    ...
	    IDIO del = idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_EXPR);
		 ...
		 */
		lo = idio_read_1_expr (handle, ic, depth);
		del = idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_EXPR);
	    }
	...
	if (idio_T_eol != e) {
		...
		if (idio_isa_pair (r) &&
		    ...
		    idio_struct_instance_set_direct (r_lo, IDIO_LEXOBJ_ST_EXPR, r);
...
static IDIO idio_read_quote (IDIO handle, IDIO lo, idio_unicode_t ic[IDIO_TEMPLATE_IC], int depth)
{
    ...
    IDIO e = idio_struct_instance_ref_direct (qlo, IDIO_LEXOBJ_ST_EXPR);
...
static IDIO idio_read_quasiquote (IDIO handle, IDIO lo, idio_unicode_t ic[IDIO_TEMPLATE_IC], int depth)
{
    ...
    IDIO e = idio_struct_instance_ref_direct (qqlo, IDIO_LEXOBJ_ST_EXPR);
...
static IDIO idio_read_unquote_splicing (IDIO handle, IDIO lo, idio_unicode_t ic[IDIO_TEMPLATE_IC], int depth)
{
    ...
    IDIO e = idio_struct_instance_ref_direct (uslo, IDIO_LEXOBJ_ST_EXPR);
...
static IDIO idio_read_unquote (IDIO handle, IDIO lo, idio_unicode_t ic[IDIO_TEMPLATE_IC], int depth)
{
    ...
    IDIO e = idio_struct_instance_ref_direct (uqlo, IDIO_LEXOBJ_ST_EXPR);
...
static IDIO idio_read_escape (IDIO handle, IDIO lo, idio_unicode_t ic[IDIO_TEMPLATE_IC], int depth)
{
    ...
    IDIO e = idio_struct_instance_ref_direct (elo, IDIO_LEXOBJ_ST_EXPR);
...
static IDIO idio_read_1_expr_nl (IDIO handle, idio_unicode_t ic[IDIO_TEMPLATE_IC], int depth, int return_nl)
{
	...
	if (moved) {
	    moved = 0;
	    idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_LINE, idio_integer (IDIO_HANDLE_LINE (handle)));
	    idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_POS, idio_integer (IDIO_HANDLE_POS (handle)));
	}
	...
	if (c == ic[0]) {
		...
		if (c == ic[1]) {
		    idio_struct_instance_set_direct (lo,
						     IDIO_LEXOBJ_ST_EXPR,
						     idio_read_unquote_splicing (handle, lo, ic, depth + 1));
		    return lo;
		}
		...
		idio_struct_instance_set_direct (lo,
						 IDIO_LEXOBJ_ST_EXPR,
						 idio_read_unquote (handle, lo, ic, depth + 1));
		return lo;
	    } else {
		/*
		...
		idio_struct_instance_set_direct (lo,
						 IDIO_LEXOBJ_ST_EXPR,
						 idio_read_word (handle, lo, c, ic));
		return lo;
	    }
	} else if (c == ic[2]) {
	    idio_struct_instance_set_direct (lo,
					     IDIO_LEXOBJ_ST_EXPR,
					     idio_read_quote (handle, lo, ic, IDIO_LIST_QUOTE (depth) + 1));
	    return lo;
	} else if (c == ic[3]) {
	    c = idio_getc_handle (handle);
	    if (idio_eofp_handle (handle)) {
		...
		idio_struct_instance_set_direct (lo,
						 IDIO_LEXOBJ_ST_EXPR,
						 idio_read_escape (handle, lo, ic, depth));
		return lo;
	    }
	} else {

	    if (idio_eofp_handle (handle)) {
		idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, idio_S_eof);
		return lo;
	    }
		...
		idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, idio_T_eol);
		return lo;
	    case IDIO_CHAR_LPAREN:
		{
		    IDIO l = idio_read_list (handle, lo, idio_T_lparen, ic, IDIO_LIST_PAREN (depth) + 1);
		    idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, l);
		    if (idio_S_nil != l &&
			IDIO_LIST_CONSTANT_P (depth) == 0) {
		...
		if (IDIO_LIST_PAREN_P (depth)) {
		    idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, idio_T_rparen);
		    return lo;
		} else {
		    /*
		...
		if (IDIO_CHAR_LBRACE == idio_peek_handle (handle)) {
			...
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 IDIO_LIST2 (idio_S_subshell, subshell));
		...
		idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, block);
		idio_hash_put (idio_src_properties, block, lo);
		return lo;
	    }
	    case IDIO_CHAR_RBRACE:
		if (IDIO_LIST_BRACE_P (depth)) {
		    idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, idio_T_rbrace);
		    return lo;
		} else {
		    /*
		...
		if (IDIO_LIST_BRACKET_P (depth)) {
		    idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, idio_T_rbracket);
		    return lo;
		} else {
		    /*
	    ...
	    case IDIO_CHAR_DOT:
		{
		    ...
		    default:
			idio_ungetc_handle (handle, c2);
			idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, idio_T_dot);
			return lo;
		    }
		}
		break;
	    case IDIO_CHAR_BACKQUOTE:
		{
		    ...
		    idio_struct_instance_set_direct (lo,
						     IDIO_LEXOBJ_ST_EXPR,
						     idio_read_quasiquote (handle,
									   lo,
									   qq_ic,
									   IDIO_LIST_QUASIQUOTE (depth) + 1));
		    return lo;
		}
	    case IDIO_CHAR_HASH:
		{
			...
			/* constants */
		    case 'f':
			idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, idio_S_false);
			return lo;
		    case 't':
			idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, idio_S_true);
			return lo;
		    case 'n':
			idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, idio_S_nil);
			return lo;
			...
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_character (handle,
									      lo,
									      IDIO_READ_CHARACTER_EXTENDED));
			return lo;
		    case IDIO_CHAR_LBRACKET:
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_array (handle, lo, ic, depth + 1));
			return lo;
		    case IDIO_CHAR_LBRACE:
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_hash (handle, lo, ic, depth + 1));
			return lo;
		    ...
		    case IDIO_CHAR_EQUALS:
			{
			    ...
			    case IDIO_CHAR_LBRACKET:
				/*
				 * NB. We'll pass idio_S_nil for {lo}
				 * to idio_read_array() to prevent
				 * unnecessary creation of lexobjs
				 */
				idio_struct_instance_set_direct (lo,
								 IDIO_LEXOBJ_ST_EXPR,
								 idio_read_array (handle, idio_S_nil, icv, depth + 1));
				return lo;
			    default:
				{
				    /*
			...
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_bitset (handle, lo, depth));
			return lo;
		    case 'b':
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_bignum_radix (handle, lo, c, 2));
			return lo;
		    case 'd':
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_bignum_radix (handle, lo, c, 10));
			return lo;
		    case 'o':
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_bignum_radix (handle, lo, c, 8));
			return lo;
		    case 'x':
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_bignum_radix (handle, lo, c, 16));
			return lo;
		    case 'U':
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_unicode (handle, lo));
			return lo;
		    case 'e':
		    case 'i':
			{
			    int inexact = ('i' == c);
			    IDIO lo = idio_read_1_expr (handle, ic, depth);
			    IDIO bn = idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_EXPR);
				...
				if (0 == inexact) {
				    idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, bn);
				    return lo;
				} else {
			...
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_pathname (handle, lo, depth));
			return lo;
		    case 'S':
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_istring (handle, lo, IDIO_LIST_STRING (depth) + 1));
			return lo;
		    case 'T':
			idio_struct_instance_set_direct (lo,
							 IDIO_LEXOBJ_ST_EXPR,
							 idio_read_template (handle, lo, IDIO_LIST_QUASIQUOTE (depth) + 1));
			return lo;
	    ...
	    case IDIO_CHAR_PERCENT:
		{
		    ...
		    case 'B':
			{
			    ...
			    idio_struct_instance_set_direct (lo,
							     IDIO_LEXOBJ_ST_EXPR,
							     idio_read_octet_string (handle,
										     lo,
										     cdelim,
										     idio_default_string_ic));
			    return lo;
			}
		    case 'P':
			{
	    ...
	    case IDIO_PAIR_SEPARATOR:
		{
			...
			if (depth) {
			    idio_struct_instance_set_direct (lo, IDIO_LEXOBJ_ST_EXPR, idio_T_pair_separator);
			    return lo;
			} else {
			    /*
...
static IDIO idio_read_expr_line (IDIO handle, IDIO closedel, idio_unicode_t ic[IDIO_TEMPLATE_IC], int depth)
{
	...
	if (skipped) {
	    skipped = 0;
	    idio_struct_instance_set_direct (line_lo,
					     IDIO_LEXOBJ_ST_LINE,
					     idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_LINE));
	    idio_struct_instance_set_direct (line_lo,
					     IDIO_LEXOBJ_ST_POS,
	...
	IDIO expr = idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_EXPR);
	...
	if (idio_S_eof == expr) {
		...
		if (idio_S_nil == IDIO_PAIR_T (re)) {
			...
			idio_struct_instance_set_direct (re_lo, IDIO_LEXOBJ_ST_EXPR, re);
			idio_hash_put (idio_src_properties, re, re_lo);
		    }
		}
		idio_struct_instance_set_direct (line_lo, IDIO_LEXOBJ_ST_EXPR, re);
		return idio_pair (line_lo, idio_S_eof);
	    } else {
		idio_struct_instance_set_direct (line_lo, IDIO_LEXOBJ_ST_EXPR, idio_S_eof);
		return idio_pair (line_lo, idio_S_eof);
	    }
...
static IDIO idio_read_block (IDIO handle, IDIO lo, IDIO closedel, idio_unicode_t ic[IDIO_TEMPLATE_IC], int depth)
{
	...
	IDIO expr = idio_struct_instance_ref_direct (line_lo, IDIO_LEXOBJ_ST_EXPR);
	...
	if (closedel == reason) {
		...
		if (idio_S_nil == r) {
#ifdef IDIO_DEBUG
		    idio_debug ("NOTICE: idio_read_block: %s", idio_struct_instance_ref_direct (line_lo, IDIO_LEXOBJ_ST_NAME));
		    idio_debug (":line %s", idio_struct_instance_ref_direct (line_lo, IDIO_LEXOBJ_ST_LINE));
...
IDIO idio_read_expr (IDIO handle)
{
	...
	expr = idio_struct_instance_ref_direct (lo, IDIO_LEXOBJ_ST_EXPR);
	if (idio_S_eof == expr) {
	    break;
	}
    }
