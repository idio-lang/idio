...
static void idio_file_handle_filename_system_error (char const *circumstance, IDIO filename, IDIO c_location)
{
    ...
    idio_error_raise_cont (idio_condition_io_filename_error_type,
...
static void idio_file_handle_malformed_filename_error (char const *msg, IDIO filename, IDIO c_location)
{
    ...
    idio_error_raise_cont (idio_condition_io_malformed_filename_error_type,
			   IDIO_LIST4 (idio_get_output_string (msh),
				       idio_get_output_string (lsh),
				       idio_get_output_string (dsh),
				       filename));
...
static void idio_file_handle_file_protection_error (char const *circumstance, IDIO filename, IDIO c_location)
{
    ...
    idio_error_raise_cont (idio_condition_io_file_protection_error_type,
			   IDIO_LIST4 (idio_get_output_string (msh),
				       idio_get_output_string (lsh),
				       idio_get_output_string (dsh),
				       filename));
...
static void idio_file_handle_filename_already_exists_error (char const *circumstance, IDIO filename, IDIO c_location)
{
    ...
    idio_error_raise_cont (idio_condition_io_file_already_exists_error_type,
			   IDIO_LIST4 (idio_get_output_string (msh),
				       idio_get_output_string (lsh),
				       idio_get_output_string (dsh),
				       filename));
...
static void idio_file_handle_filename_not_found_error (char const *circumstance, IDIO filename, IDIO c_location)
{
    ...
    idio_error_raise_cont (idio_condition_io_no_such_file_error_type,
			   IDIO_LIST4 (idio_get_output_string (msh),
				       idio_get_output_string (lsh),
				       idio_get_output_string (dsh),
				       filename));
...
void idio_file_handle_dynamic_load_error (char const *circumstance, char const *msg, IDIO filename, IDIO detail, IDIO c_location)
{
    ...
    idio_error_raise_cont (idio_condition_rt_load_error_type,
			   IDIO_LIST4 (idio_get_output_string (msh),
				       idio_get_output_string (lsh),
				       idio_get_output_string (dsh),
				       filename));
...
static IDIO idio_file_handle_open_from_fd (IDIO ifd, IDIO args, int h_type, char const *func, char const *def_mode_str, size_t const dms_len, int def_mode, int plus_mode)
{
	...
	if (idio_S_nil == name) {
		 ...
		 */
		if (free_name_C) {
		    IDIO_GC_FREE (name_C, name_C_len);
		}
	...
	if (free_mode_C) {
	    IDIO_GC_FREE (mode_C, mode_C_len);
	}
...
IDIO idio_open_file_handle_C (char const *func, IDIO filename, char const *pathname, size_t const pathname_len, int free_pathname, char const *mode_str, size_t const mode_str_len, int free_mode_str, int user_mode)
{
	...
	if (free_mode_str) {
	    IDIO_GC_FREE (mode_str, mode_str_len);
	}
...
void idio_free_file_handle (IDIO fh)
{
    ...
    idio_free (IDIO_FILE_HANDLE_BUF (fh));
    idio_free (IDIO_HANDLE_STREAM (fh));
}
...
IDIO idio_load_dl_library (char const *filename, size_t const filename_len, char const *libname, size_t const libname_len, IDIO eenv)
{
    ...
    IDIO_GC_FREE (mod_name, mod_len + 1);
...
char *idio_find_libfile_C (char const *file, size_t const file_len, size_t *liblenp)
{
	...
	idiolib_copy_C = idio_string_as_C (IDIOLIB, &idiolib_copy_C_len);

	/*
	 * Use idiolib_copy_C_len + 1 to avoid a truncation warning -- we're just
	 * seeing if idiolib_copy_C includes a NUL
	 */
	size_t const C_size = idio_strnlen (idiolib_copy_C, idiolib_copy_C_len + 1);
	if (C_size != idiolib_copy_C_len) {
	    /*
	...
	while (! done) {
		 ...
		 * {lne} which points at the end of the current value
		 * of {libname}.  That is, it points at the '\0' at
		 * the end of "/path/to/".
		 */
		...
		for (; NULL != fe->reader; fe++) {
			...
			if (idio_dl_read == fe->reader) {
				...
				if (stat (libname, &sb) ||
				    ...
				    /*
				     * not strictly an error -- maybe
				     * the correct hierarchy in in
				     * another directory on IDIOLIB
				     * and this is just cruft left
				     * around
				     */
				    if (9 != mod_len ||
					 ...
					 * expected to exist.  If a
					 * user has created a
					 * bootstrap extension and
					 * messed up then, well, we're
					 * all in trouble
					 */
#ifdef IDIO_DEBUG
				...
				if (-1 == read_r) {
				     ...
				     * Test Case: ??
				     *
				     * How to get a (previously good)
				     * file descriptor to fail for
				     * read(2)?
				     */
				...
				if (read_r != sb.st_size) {
				     ...
				     * Test Case: ??
				     *
				     * Our file has changed size since
				     * the stat(2) above?
				     */
				    char em[BUFSIZ];
...
char *idio_find_libfile (IDIO file, size_t *libfile_C_lenp)
{
    ...
    if (free_file_C) {
	IDIO_GC_FREE (file_C, file_C_len);
    }
...
IDIO idio_load_file_name (IDIO filename, IDIO eenv)
{
	...
	IDIO_GC_FREE (libfile, libfilelen);
    }
