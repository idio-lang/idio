...
{
    char *pathname_C = idio_find_file (file);

    if (NULL == pathname_C) {
	fprintf (stderr, "idio_load_file: '%s' not found\n", file);
	return 0;
    }

    idio_collector_t *collector = IDIO_COLLECTOR (f);

    IDIO pathname = idio_string_C (f, pathname_C);
    
    IDIO inp = idio_open_handle (f, pathname, "r");
    IDIO_ASSERT (inp);
    
    if (idio_S_nil == inp) {
	char em[BUFSIZ];
	sprintf (em, "open-output-file \"%s\": %s", pathname_C, strerror (errno));
	fprintf (stderr, "%s\n", em);
	free (pathname_C);
	return 0;
    }
    
    IDIO_TYPE_ASSERT (opaque, inp);
    IDIO oinp = idio_array_get_index (f, collector->input_port, 0);
    idio_frame_stack_push (f, oinp);
    idio_array_insert_index (f, collector->input_port, inp, 0);

    char *output_C = "/dev/null";
    
    IDIO output = idio_string_C (f, output_C);
    IDIO outp = idio_open_handle (f, output, "w");
    IDIO_ASSERT (outp);
    
    if (idio_S_nil == outp) {
	char em[BUFSIZ];
	sprintf (em, "open-output-file \"%s\": %s", output_C, strerror (errno));
	fprintf (stderr, "%s\n", em);
	free (pathname_C);
	return 0;
    }

    IDIO_TYPE_ASSERT (opaque, outp);
    /* idio_array_unshift (f, collector->output_port, outp); */

    fprintf (stderr, "idio_load_file: %s\n", pathname_C);
    idio_repl (f, inp, 0);

    /* idio_array_shift (f, collector->output_port); */
    idio_array_insert_index (f, collector->input_port, oinp, 0);
    idio_frame_stack_pop (f);	/* oinp */
